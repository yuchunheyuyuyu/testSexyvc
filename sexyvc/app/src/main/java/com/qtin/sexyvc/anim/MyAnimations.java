...
public class MyAnimations {
	...
	public static void startAnimationsIn(ViewGroup viewgroup, int durationMillis) {
		for (int i = 0; i < viewgroup.getChildCount(); i++) {
			...
			if(i==1){
				int x=-(CustomApplication.screenSize.x/2- DeviceUtils.dip2px(viewgroup.getContext(),110));
				animation = new TranslateAnimation(x, 0F, mlp.bottomMargin, 0F);
			}else{
				int x=(CustomApplication.screenSize.x/2- DeviceUtils.dip2px(viewgroup.getContext(),110));
				animation = new TranslateAnimation(x, 0F, mlp.bottomMargin, 0F);
			}
	...
	public static void startAnimationsOut(ViewGroup viewgroup, int durationMillis) {
		for (int i = 0; i < viewgroup.getChildCount(); i++) {
			...
			if(i==1){
				int x=-(CustomApplication.screenSize.x/2- DeviceUtils.dip2px(viewgroup.getContext(),110));
				animation = new TranslateAnimation(0F, x, 0F,mlp.bottomMargin);
			}else{
				int x=(CustomApplication.screenSize.x/2- DeviceUtils.dip2px(viewgroup.getContext(),110));
				animation = new TranslateAnimation(0F,x, 0F,mlp.bottomMargin);
			}
	...
	// 文字的淡入
	public static Animation getFadeInAnimation(int durationMillis) {
		Animation miniAnimation = new AlphaAnimation(0,1);
		miniAnimation.setDuration(durationMillis);
		miniAnimation.setFillAfter(true);
		return miniAnimation;
	}
	// 文字的淡出
	public static Animation getFadeOutAnimation(int durationMillis) {
		Animation miniAnimation = new AlphaAnimation(1,0);
		miniAnimation.setDuration(durationMillis);
		miniAnimation.setFillAfter(true);
		return miniAnimation;
	}
