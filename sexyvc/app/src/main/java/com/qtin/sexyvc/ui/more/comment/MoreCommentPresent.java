...
public class MoreCommentPresent extends BasePresenter<MoreCommentContract.Model,MoreCommentContract.View> {
    ...
    public void queryFundComment(long fund_id,final long comment_id,int auth_state){
        mModel.queryFundDetail(mModel.getToken(),fund_id,comment_id,15,auth_state)
    ...
    public void queryInvestorComment(long investor_id,final long comment_id,int auth_state){
        mModel.queryInvestorDetail(mModel.getToken(),investor_id,comment_id,15,auth_state)
                .subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(new Action0() {
                    @Override
